...
const Editor = dynamic(() => import("@monaco-editor/react"), {
  ssr: false,
  loading: () => (
    <div className="h-[200px] flex items-center justify-center text-muted-foreground">Loading editor...</div>
  ),
})
      ...
      <div className="rounded-lg border border-border bg-background">
        <div className="flex items-center justify-between p-1 border-b border-border">
          ...
          <div className="flex space-x-1">
            ...
            <Button size="sm" variant="outline" onClick={handleCancel} className="h-6 px-2 text-[10px] bg-transparent">
      ...
      <div className="h-[calc(100vh-16rem)] min-h-[200px] max-h-[800px] overflow-y-auto">
        <Highlight code={code} language={language}>
          {({ className, style, tokens, getLineProps, getTokenProps }) => (
            <pre
              className={className + " rounded-lg p-2 text-[10px]"}
              style={{
                ...style,
                whiteSpace: "pre-wrap",
