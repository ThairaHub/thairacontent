...
export function PreviewPane({ messages, activeView, provider, onFilesSelected }: PreviewPaneProps) {
  ...
  if (activeView === "code") {
    return (
      ...
      RawView(isMobileSidebarOpen, setIsMobileSidebarOpen, contentBlocks, selectAllFiles, allFiles, clearSelection, selectedFiles, versions, activeVersionId, handleVersionChange, handleFileClick, handleFileSelection, provider, openTabs, activeTab, setActiveTab, closeTab, activeTabContent, updateContentBlock)
    )
  } else {
    return (
...
function RawView(isMobileSidebarOpen: boolean, setIsMobileSidebarOpen: React.Dispatch<React.SetStateAction<boolean>>, contentBlocks: CodeStructBlock[], selectAllFiles: () => void, allFiles: CodeStructBlock[], clearSelection: () => void, selectedFiles: Set<string>, versions: ContentVersion[], activeVersionId: string | null, handleVersionChange: (versionId: string) => void, handleFileClick: (filename: string) => void, handleFileSelection: (filename: string, selected: boolean) => void, provider: string, openTabs: string[], activeTab: string | null, setActiveTab: React.Dispatch<React.SetStateAction<string | null>>, closeTab: (filename: string, e?: React.MouseEvent) => void, activeTabContent: CodeStructBlock | null, updateContentBlock: (filename: string, newContent: string) => void) {
              ...
              <DropdownMenuContent className="w-40 bg-background border border-border">
                {versions.map((version) => (
                  <DropdownMenuItem
                    ...
                    className={`cursor-pointer hover:bg-secondary/80 ${activeVersionId === version.id ? "bg-secondary text-secondary-foreground" : ""}`}
                  >
          ...
          <div className="border-b border-border bg-background">
            <div className="flex items-center overflow-x-auto scrollbar-hide">
              {openTabs.map((tabFile) => (
                <div
                  key={tabFile}
                  className={`flex items-center px-2 sm:px-3 py-2 border-r border-border cursor-pointer min-w-0 flex-shrink-0 ${activeTab === tabFile
                  ...
                  <Card className="p-2 sm:p-4 bg-message-bg border-border relative">
                    ...
                    <div className="overflow-hidden">
                      <ContentViewer
                        ...
                        platform={activeTabContent.language === "twitter"
                          ? "X (Twitter)"
